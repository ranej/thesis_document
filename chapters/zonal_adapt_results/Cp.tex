...
Note that meshes with same in-plane resolution do not show significant variations for different spanwise resolution.
...
Note that the peak in $C_p$ corresponds with the low pressure associated with a vorticity build-up.
...
For phase $\psi=210^\circ$, a peak in $C_p$ is observed for all meshes apart from M0 mesh.
...
For phase $\psi=240^\circ$, a peak in $C_p$ can be seen for all meshes.
...
Since we have already seen through spanwise vorticity plots and Cp that the spanwise resolution of the meshes considered does not affect the solution significantly, we only show data from meshes with the second highest spanwise resolution for brevity. 
